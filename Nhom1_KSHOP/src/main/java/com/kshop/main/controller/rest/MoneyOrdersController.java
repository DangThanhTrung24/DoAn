...
public class MoneyOrdersController {
	...
	public List<MoneyOrders> getAllMoneyOrders() {
		...
		return hiddenPassword(moneyOrdersService.findAll());
	}
	...
	public APIResponse addMoneyOrders(@RequestBody MoneyOrders moneyOrders) {
	    ...
	    if(!orderPost.get().getOrder_status().equals("Complete")) {
	    ...
	    MoneyOrders moneyOrdersDB = moneyOrdersService.selectByUserIdAndOrderId(userPost.get().getId(), moneyOrders.getOrder().getId());
